...
| 2026-08-31 | Playwright 验收 | styles/global.css, views/OverviewView.vue | Playwright 实机验收(4 视口 x 8 页面 x 亮/暗 + 对比度 + 键盘焦点 + 375 溢出). 修复: 亮色 --lb-muted #6b7280->#5b6b83(4.8:1 满足 AA); Overview 网格 minmax(0) 防长任务ID撑破; view-grid--two 去掉 360px 下限并在 1180 折叠单列(/schedules 375 溢出修复); el-table 表头/空态文字提至 fg-soft(3.08->7.58); 长任务ID省略号. 结果: 暗色全 OK, 375 全 OK, 表格对比度 OK, 焦点 11/14+EP控件wrapper态 |
| 2026-08-31 | 验收修复 | styles/global.css | 修复 --lb-primary 变量未定义 (12+ 组件引用但 :root 中缺失, 导致焦点环/FilterChips/TaskTimeline 等透明): 亮色 #1e40af + 暗色 #60a5fa; body background 从简写拆为 background-color + background-image 使对比度检测工具识别真实背景; 新增 EP 组件 (el-select/el-input/el-radio-button/el-checkbox) focus-visible 焦点环. 验收: 对比度 light 全 OK (body 1.42->13.65, page-title 1.18->16.51, muted 3.88->5.01), 焦点 11/14->13/14, 375 溢出 8/8 OK |
