...
| 报告输出 | Markdown | `app/reports/markdown.py`、`reports/` |
| 测试框架 | pytest | `tests/` |
...
| `app/core/plans.py` | 默认评测计划 `gateway_baseline_v1` 与指标中文名称/说明。 |
...
当前默认计划 `gateway_baseline_v1` 包含：

| 顺序 | 指标 ID | 中文名 | 说明 |
|---:|---|---|---|
| 1 | `connectivity` | 连通性 | 检查基础调用、JSON、内容、`finish_reason`、`usage`。 |
| 2 | `latency_breakdown` | 延迟拆解 | 通过流式请求采集 TTFT、端到端耗时、估算 TPS。 |
...
| 5 | `concurrency` | 并发承载 | 按 `concurrency_levels` 并发请求，统计成功、失败、延迟。 |
| 6 | `rate_limit` | 限流行为 | 复用并发探测结果，记录 429 等限流信号。 |
| 7 | `error_handling` | 错误处理 | 构造无效 key、无效模型、空输入、非法参数、上下文超限等场景。 |
| 8 | `token_usage_accuracy` | Token 用量观测（Usage） | 观察 usage 字段，并与本地启发式估算辅助对比。 |
| 9 | `stream_spec` | 流式规范性 | 检查 SSE 是否可解析、是否有 `[DONE]`、`finish_reason`、流式 usage。 |
...
| `app/reports/markdown.py` | 生成 Markdown 报告，包含 LLM 分析、任务概览、指标仪表盘、指标总表、明细和附录。 |
...
5. `concurrency` 与 `rate_limit` 共享一组并发请求结果。
6. 报告分析模型未配置或失败时，基础任务仍完成，报告中记录跳过/失败原因。
7. 最终 API 返回 `TaskResult` JSON，同时落库任务 JSON 并生成 Markdown 报告。
...
- 不要提交 `data/models.json`、`data/tasks/`、`reports/`，它们可能包含密钥或运行产物。
